document.addEventListener('DOMContentLoaded', function(){
  ...
  // Base path for per-subject daily JSONL files; we'll append `<subject>.today.jsonl`
  const basePath = 'https://raw.githubusercontent.com/manh-td/arxiv-recent/refs/heads/main/data/';

  async function loadRemotePapers(subject = 'cs.SE') {
    try {
      const urlToFetch = basePath + (subject || 'cs.SE') + '.today.jsonl';
      ...
      for (const line of lines) {
        try {
          ...
          // Prefer summary/abstract when available (arXiv uses `summary`) and truncate
          let desc = obj.summary || obj.abstract || '';
          const cleanedDesc = desc ? desc.replace(/<[^>]*>/g, '') : '';
          const MAX_DESC = 300;
          const shortDesc = cleanedDesc.length > MAX_DESC ? (cleanedDesc.slice(0, MAX_DESC).trim() + '...') : cleanedDesc;
          desc = (typeof escapeHtml === 'function') ? escapeHtml(shortDesc) : shortDesc;

          // Authors can be an array; include published date if present
          const authorsRaw = Array.isArray(obj.authors) ? obj.authors.join(', ') : (obj.authors || obj.author || '');
          const published = obj.published || obj.updated || '';
          const pubDate = published ? (published.split('T')[0]) : '';
          const authors = authorsRaw ? ('<strong>Authors:</strong> ' + authorsRaw + (pubDate ? ' — <em>' + pubDate + '</em>' : '')) : (pubDate ? ('<em>' + pubDate + '</em>') : '');

          // Prefer various URL fields; arXiv often exposes `pdf_url` or `id` (abs URL)
          const url = obj.url || obj.pdf || obj.pdf_url || obj.arxiv_url || obj.source || obj.link || obj.id || '';
  ...
  if (applyDateFilter && datePicker) {
    applyDateFilter.addEventListener('click', async () => {
      // `selectedDate` is actually the subject select now; default to 'cs.SE'
      const subject = datePicker.value || 'cs.SE';
      const remote = await loadRemotePapers(subject);
      if (remote && remote.length) {
        const mapped = remote.map((p) => ({
          title: p.title || 'No title',
          desc: p.desc || p.abstract || '',
        ...
        // No data for subject: clear to sentinel only
        renderCards([]);
      }
    });
  }
